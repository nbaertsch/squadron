...
class TestSecurityReviewAgentRole:
    ...
    def test_security_review_agent_config_has_issue_context(self):
        ...
        assert "issue" in agent_config.lower(), (
            "Security-review agent should handle issue assignment"
        )

        # The config should have instructions for delegation
        assert "@squadron-dev" in agent_config, (
            "Security-review agent should delegate via @ mentions"
        )

        # Filter out ❌ example sections (showing what NOT to say)
        ...
        # The config should NOT claim to implement fixes (excluding ❌ examples)
        problematic_phrases = [
        ...
        for phrase in problematic_phrases:
            assert phrase not in filtered_config, (
                f"Security-review agent should not claim '{phrase}'"
            )
    ...
    def test_security_review_agent_config_separates_pr_and_issue_workflows(self):
        ...
        has_conditional = any(
            indicator.replace(".*", " ") in agent_config.lower() for indicator in context_indicators
        )
