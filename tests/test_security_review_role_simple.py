...
def test_security_review_agent_config_prevents_fix_claims():
    ...
    has_review_only = any(indicator in agent_config.lower() for indicator in review_only_indicators)
    ...
    has_delegation_req = (
        "delegate" in agent_config.lower() and "must delegate" in agent_config.lower()
    )
    ...
    all_pass = (
        has_issue_context
        and has_delegation
        and len(fix_claims) == 0
        and has_review_only
        and handles_both
        and has_delegation_req
    )

    print(f"\nOVERALL: {'PASS' if all_pass else 'FAIL'}")

    assert all_pass, "Security-review agent config does not meet requirements"
